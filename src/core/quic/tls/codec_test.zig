...
//! QUIC-valid ClientHello, and round-trip the server flight. The crypto-core
//! vectors (ECDHE 8bd4054f..., the handshake secrets) are the same ones schedule.zig
//! and keyshare.zig already pin, so a green test here proves the codec surfaces
//! byte-exact inputs to that validated core.
...
/// supported_groups=x25519, signature_algorithms=ecdsa_secp256r1_sha256, and an
/// (empty) quic_transport_parameters - everything the server's policy requires.
...
test "build emits a parseable ServerHello whose key_share round-trips to a shared secret" {
    ...
    const signer = try @import("sign.zig").Signer.fromSeed([_]u8{0x42} ** 32);
    ...
    const built = try flight.build(&out, testing.allocator, &transcript, view, cfg);
    ...
    // The handshake secrets are non-empty (schedule ran) and the application
    // secrets differ from them (a later derivation point).
