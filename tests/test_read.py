...
def test_post_content_length() -> None:
    ...
    assert isinstance(events[0], zhttp.Request)
...
def test_zero_content_length_has_no_data() -> None:
    events = parse_request(b"POST / HTTP/1.1\r\nContent-Length: 0\r\n\r\n")
    assert not any(isinstance(e, zhttp.Data) for e in events)
    assert isinstance(events[-1], zhttp.EndOfMessage)
...
def test_body_split_across_feeds() -> None:
    ...
    assert isinstance(conn.next_event(), zhttp.EndOfMessage)
...
def test_response_parsing() -> None:
    ...
    assert isinstance(resp, zhttp.Response)
...
def test_response_until_close() -> None:
    ...
    assert isinstance(conn.next_event(), zhttp.Response)
...
def test_eof_on_empty_connection_yields_closed() -> None:
    conn = zhttp.Connection(zhttp.SERVER)
    conn.receive_data(b"")
    assert conn.next_event() is zhttp.ConnectionClosed
...
def test_remote_is_subclass_of_protocol_error() -> None:
    assert issubclass(zhttp.RemoteProtocolError, zhttp.ProtocolError)
    assert issubclass(zhttp.LocalProtocolError, zhttp.ProtocolError)
